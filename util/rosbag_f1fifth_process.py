...
class F1TenthDataCollector(Node):
    def __init__(self, csv_filename, dt):
...
def main():
    ...
    parser.add_argument("dt", type=float, help="Time interval between samples in seconds (e.g., 0.02 for 50Hz)")

    args = parser.parse_args()

    rclpy.init()

    collector = F1TenthDataCollector(args.csv_filename, args.dt)

    try:
        collector.get_logger().info(f"Starting data collection with {args.dt}s timer. Press Ctrl+C to stop.")
